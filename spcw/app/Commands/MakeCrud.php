...
class MakeCrud extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        foreach($spec['dataEntities'] as $entitySpec) {
            ...
            if ($entitySpec['type'] == 'Entity') {
                ...
                $menuEntities[] = [
                    'title' => plural(humanize(decamelize($entitySpec['name']))),
                    'url'   => '/' . dasherize(decamelize($entitySpec['name'])),
                ];
            }
        }
    ...
    protected function generateAppEntity(string $resourceName, array $entitySpec, array $symbolTable) {
        // Example: "User" => "User" / "Users"
        $className   = ucfirst($resourceName);
        $tablePlural = plural(strtolower($className));
        $baseRoute = dasherize(decamelize($className));
        ...
        $this->generateModel($className, $tablePlural, $useDates, $fields, $wantEntity);
        $this->generateController($className, $baseRoute, $relations);

        if ($wantMigration) {
            $this->generateMigration($className, $tablePlural, $useDates, $fields);
        }
        if ($wantSeeder) {
            $seeds = $entitySpec['seeds'] ?? [];
            $this->generateSeeder($className, $relations, $seeds);
        }
        if ($wantEntity) {
            $this->generateEntity($className, $useDates, $fields);
        }

        // Generate views (new code for placeholders)
        $this->generateViews($className, $baseRoute, $fields);
    }

    /**
     * Generate the Model class.
     */
    protected function generateModel(string $className, string $tablePlural, bool $useDates, array $fields, bool $wantEntity)
    {
        ...
        $modelReturnType = $wantEntity
            ? "\\App\\Entities\\{$className}Entity::class"
            : "'array'";
     ...
     */
    protected function generateController(string $className, string $baseRoute, array $relations)
    {
        $controllerName = $className;  // e.g. "User"
...
PHP;
     ...
     */
    protected function generateMigration(string $className, string $tablePlural, bool $useDates, array $fields)
    {
        // e.g. "2025-01-31-123456_CreateUsersTable.php"
        $timestamp     = date('Y-m-d-His');
        $migrationName = "Create" . ucfirst($tablePlural) . "Table";
     ...
     * Generate the Seeder.
     */
    protected function generateSeeder(string $className, array $relations, array $seeds = [])
    {
        ...
        \$temp{$relatedClassName}Model = model('{$relatedClassName}Model');
        if (\$temp{$relatedClassName}Model->useAutoIncrement) {
            \$seedData = populateForeignKeys(\$temp{$relatedClassName}Model, \$seedData, '$fieldName');
        }
...
PHP;
     ...
     */
    protected function generateEntity(string $className, bool $useDates, array $fields)
    {
        ...
        if ($secretPropertyGetters) {
            ...
            $stub = str_replace(
                "use DisplayNameAccess",
                "use MaskingEntity, DisplayNameAccess",
                $stub
            );
        }
     ...
     * Generate the Views.
     */
    protected function generateViews(string $className, string $baseRoute, array $fields)
    {
        $viewsPath = APPPATH . "Views/{$className}";
        ...
        $tableRowCells = $this->generateTableRowCells($fields, $baseRoute, 'compact');
    ...
    protected function generateTableHeaders(array $fields, ?string $projection = null): string
    {
        ...
        foreach ($fields as $fieldName => $expectoEntityField) {
            ...
            $headers .= "<th{$align}>" . ucfirst($fieldName) . "</th>";
        }
        return $headers;
    }
    ...
    protected function generateTableRowCells(array $fields, string $baseRoute, ?string $projection = null): string
    {
        ...
        foreach ($fields as $fieldName => $expectoEntityField) {
            ...
            if ($expectoEntityField->primaryDisplayName) {
                $rowCells[] = "<td{$align}><a href=\"<?= site_url('$baseRoute/show/' . \$item->id) ?>\"><?= {$expectoEntityField->escPhp()} ?></a></td>";
            } else {
...
HTML;
    ...
    protected function generateFormFields(array $fields, string $mode): string
    {
        ...
        $str = '';
        foreach ($fields as $fieldName => $expectoEntityField) {
            // Skip 'id' if it's your primary key
            if ($fieldName === 'id') {
                continue;
            }
...
HTML;
    ...
    protected function getPrimaryKeyField(array $fields): array
    {
        foreach ($fields as $fieldName => $props) {
            if (array_key_exists('primaryKey', $props) && $props['primaryKey']) {
